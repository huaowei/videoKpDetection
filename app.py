    ...
    try:
        ...
        for program in programs:
            if program == "3_match_sj.py":
                output = subprocess.check_output(["python", program, video_name], encoding='gbk')
                # 编写正则表达式模式来匹配需要的内容
                ...
                if segments is not None:
                    subprocess.run(["python", program, video_name] + [f"{start}:{end}" for start, end in segments], check=True)
                else:
                    return jsonify({'error': 'segments is not defined. Make sure to run 3_match_sj.py first.'})
            else:
                subprocess.run(["python", program, video_name], check=True)

        return jsonify({'message': 'All programs executed successfully', 'segments': segments, 'res_list_text': res_list_text, 'res_tp_text': res_tp_text})
...
def upload_train_mp4():
    ...
    if allowed_file(fname):
        ...
        # 调用子进程运行帧图片提取脚本
        subprocess.run(["python", 'get_train_imgs.py', os.path.splitext(fname)[0]], check=True)
    else:
        # 保存视频文件
        video_path = os.path.join(app.config['UPLOAD_TRAIN_VIDEO_FOLDER'], 'default_name')
        file.save(video_path)
