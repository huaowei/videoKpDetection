...
# 实例化数据库对象sor
sor = read_db.DB(host='47.102.204.29', user='shuishandb', password='midStage2022', database='self_adaptive_learning',
...
def spilt_kp_word(stopwords_filepath):
    sql = 'SELECT kp_id, kp_name FROM kp_knowledge_point_zr WHERE type = 4 ;'
...
# 强制选择方式
flag_have_first_title = False
# if flag_have_first_title:
#     knowledge_points = spilt_kp_word_true(result_lists,stopwords_filepath)
# else:
...
print(data_)
# 添加筛选机制
result = list([value[1] for inner_dict in eval(data_).values() for value in inner_dict['knowledge_points'].values()])
# print(result)
query_test = "SELECT parent_id FROM kp_knowledge_point_zr WHERE kp_id = %s;"
count_kp_parent = {}
for item in result:
...
query_l2 = "SELECT kp_id FROM kp_knowledge_point_zr WHERE parent_id = %s;"
...
for key, inner_dict in eval(data_).items():
    ...
    for inner_key, value in inner_dict['knowledge_points'].items():
        if value[1] in [item['kp_id'] for sublist in effective_list for item in sublist]:
...
key_list = [(int(key.split('_')[0])*internal_time, int(key.split('_')[1].split('.')[0])*internal_time) for key, _, _ in sorted_data]
...
def dict_convert_tuple(selected_knowledge_points):
    ...
    for item in kp_dicts_list:
        query_1 = "SELECT kp_id FROM kp_knowledge_point_zr WHERE kp_name = %s AND type = 4;"
        kp_name_to_search = item[1]
        kp_id = sor.get_one_2(query_1, (kp_name_to_search,))
        # print(kp_id)

        # 通过kp_id查主题知识点和知识对象
        query_2 = "SELECT kp_name FROM kp_knowledge_point_zr kp, ( SELECT @id as id, (SELECT @id := parent_id FROM kp_knowledge_point_zr WHERE kp_id = @id) as pid, @l := @l + 1 as level FROM kp_knowledge_point_zr, (SELECT @id := %s, @l := 0) b WHERE @id > 1 ) a WHERE kp.kp_id = a.pid AND kp.kp_name is not NULL;"
        query_2_params = kp_id['kp_id']
        kp_name_list = sor.get_all_2(query_2, (query_2_params,))
        # print(kp_name_list)

        # 通过kp_id查知识单元
        query_0 = "SELECT ke.knowledge_element_name FROM kp_knowledge_point_zr kp, kp_ke_relationship kkr, kp_knowledge_element ke WHERE kp.kp_id = kkr.chlid_id AND kkr.parent_id = ke.knowledge_element_id AND kp_id = ( SELECT kp.kp_id FROM( SELECT @id as id, (SELECT @id := parent_id FROM kp_knowledge_point_zr WHERE kp_id = @id) as pid, @l := @l + 1 as level FROM kp_knowledge_point_zr, (SELECT @id := %s, @l := 0) b WHERE @id > 0  LIMIT 3 ) kp_tmp, kp_knowledge_point_zr kp WHERE kp_tmp.id = kp.kp_id AND type = 1 ORDER BY level );"
    ...
    # 合并三个列表
    tuple_kp_dicts_list = unrepeated_elem_to_object_dict_list + unrepeated_object_to_main_dict_list + main_to_exact_dict_list
    ...
    try:
        with open(file_name, 'w', encoding='utf-8') as file:
            for tuple_kp_dict in tuple_kp_dict_list:
                file.write(tuple_kp_dict['type1'] + ',' +
